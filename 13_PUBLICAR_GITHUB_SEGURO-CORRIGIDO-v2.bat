...
echo Ele nunca deve adicionar .env real, .netlify, node_modules ou dist. O .env.example e permitido.
echo.
...
for /f "delims=" %%F in ('git ls-files') do (
  ...
  rem .env.example e propositalmente publico: contem apenas placeholders/documentacao.
  if /I "!TRACKED:~0,5!"==".env." if /I not "!TRACKED!"==".env.example" goto :blocked_file
